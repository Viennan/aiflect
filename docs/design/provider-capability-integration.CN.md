...
这样做的原因是，火山方舟 Responses API 支持默认存储 response、`previous_response_id` 和 `caching`，但这些能力具有 provider 生命周期、过期时间和迁移限制。它们应通过显式配置、`provider_options` 或 adapter 优化策略暴露，而不是让用户误以为 `vatbrain` 自身维护了远端会话真值。v0.3 暂不把 cache policy 或远端过期时间作为通用 core 字段。
...
为了表达 provider-side previous response/store，而不破坏 Full-context First，可以在 generation request 中加入 `remote_context` 类型的 hint：
...
`store` 只描述本轮 response 是否请求 provider 存储。`store=None` 表示不显式指定，交由 provider 默认行为决定。使用某个 `previous_response_id` 时，需要保证该 id 对应 response 在生成时已被 provider 存储；本轮的 `store` 仅影响本轮 response 未来是否可作为 previous response 被引用。
...
使用 `previous_response_id` 的前提是被引用 response 在生成时已被 provider 存储。本轮 `RemoteContextHint.store` 是否为 `true` 不会修复旧 id 的可访问性；它只影响本轮 response 是否便于未来被引用。
