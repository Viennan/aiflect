...
Anthropic extended thinking 不引入新的 core 字段。`vatbrain` 使用既有 `ReasoningConfig` 表达 reasoning 请求，并由 Anthropic adapter 转换为 Messages API 的 `thinking` 与 `output_config.effort`：
