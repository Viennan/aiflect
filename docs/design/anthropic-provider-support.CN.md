...
Anthropic Claude Messages API 是无状态 messages 调用面，支持文本、图片理解、工具调用、streaming、extended thinking、structured output 与 prompt caching。`aiflect` 的核心 generation 模型已经具备 full-context `Item` 序列、user-executed function tools、stream event、usage、capability、provider-native snapshot 和 `RemoteContextHint`。

本设计将 Anthropic provider 纳入 `aiflect` provider adapter 体系，同时避免把 Anthropic cache 或 Messages API 的 provider-specific 细节提升为通用 core 字段。

相关需求记录见 [REQ-2026-06-python-anthropic-adapter.CN.md](../requirements/REQ-2026-06-python-anthropic-adapter.CN.md)、[REQ-2026-06-python-anthropic-structured-output.CN.md](../requirements/REQ-2026-06-python-anthropic-structured-output.CN.md) 与 [REQ-2026-06-python-anthropic-reasoning.CN.md](../requirements/REQ-2026-06-python-anthropic-reasoning.CN.md)，Python 实现计划见 [anthropic-adapter.CN.md](../impls/python/anthropic-adapter.CN.md)。

## Design Philosophy

### Full-context First 不变

Anthropic adapter 必须继续要求用户传入完整 `items` 序列。Anthropic prompt cache 只降低重复前缀成本，不成为 `aiflect` 的上下文事实来源。
...
Anthropic 支持 client tools、server tools、MCP、web search、code execution 和 SDK Tool Runner。`aiflect` 当前通用 core 只抽象 user-executed function tools，且不自动执行工具。因此 Anthropic MVP 只支持 client-side function tools：
...
Anthropic structured outputs 不引入新的 core 字段。`aiflect` 继续使用既有 `ResponseFormat` 表达 JSON Schema structured output，由 Anthropic adapter 在 provider-local mapper 中转换为 Messages API 的 `output_config.format`：
...
Anthropic Python SDK 的 `messages.parse()` 可作为体验参考，但 adapter 不调用该 helper。这样 request mapping、response mapping、streaming、provider-native snapshot 与 Python Pydantic helper 都仍走 `aiflect` 的统一路径。
...
### Reasoning Uses Existing ReasoningConfig

Anthropic extended thinking 不引入新的 core 字段。`aiflect` 使用既有 `ReasoningConfig` 表达 reasoning 请求，并由 Anthropic adapter 转换为 Messages API 的 `thinking` 与 `output_config.effort`：

```text
ReasoningConfig(mode="disabled"|"none")
  -> thinking={"type": "disabled"}

ReasoningConfig(budget_tokens=...)
  -> thinking={"type": "enabled", "budget_tokens": ...}

ReasoningConfig(mode="auto"|"enabled"|"adaptive", effort=...)
  -> thinking={"type": "adaptive"}, output_config={"effort": ...}
```

`ReasoningConfig.summary` 只映射 Anthropic summary display 控制：`"auto"` / `"summarized"` -> `display="summarized"`，`"none"` / `"omitted"` -> `display="omitted"`。`include_trace` 与 `reasoning.provider_options` 暂不支持。

Anthropic 的 `output_config` 同时承载 structured output 与 reasoning effort。adapter 因此必须把 `ResponseFormat -> output_config.format` 与 `ReasoningConfig.effort -> output_config.effort` 作为 adapter-owned 参数安全合并，而不是允许用户通过 `provider_options["output_config"]` 绕过统一编程模型。

Active extended thinking 与 assistant prefill、`temperature`、`top_k` 和 forced tool choice 不兼容；`top_p` 如显式设置，应保持在 Anthropic 支持的区间内。手动 `budget_tokens` 还必须小于 `max_tokens`，实际模型是否支持 manual budget 或某个 effort 值仍以模型能力和 provider 响应为准。
...
- `ReasoningConfig`：extended thinking mode、effort、budget 和 summary display。
...
- 将 `ReasoningConfig` 映射为 Anthropic `thinking` 与 `output_config.effort`。
...
其中 `custom_tools=False` 指 aiflect 的 `FunctionToolType.CUSTOM`，不是指用户自定义 function tool。
...
`aiflect` 明确不自动执行工具。SDK Tool Runner 会运行工具并继续提交结果，属于 agent loop 行为，不属于 provider adapter 的职责。

### 为什么不使用 Anthropic SDK `messages.parse()`？

`messages.parse()` 是 Python SDK 的便捷层，而 `aiflect` 已有 provider-neutral `ResponseFormat` 与 Pydantic structured output helper。adapter 直接构造 `output_config.format`，可以保持 request/response/stream/snapshot 路径统一，也避免把 Anthropic SDK 的 parse shortcut 变成新的 provider-specific 编程模型。

### 为什么不直接暴露 Anthropic `thinking` / `output_config`？

`ReasoningConfig` 与 `ResponseFormat` 已经覆盖当前需要的跨 provider 语义。`thinking` 和 `output_config` 是 Anthropic transport 字段，如果通过 `provider_options` 与通用字段同时进入，会产生双来源冲突。adapter 将这些字段作为 provider-local 映射结果维护，用户侧通过 capability 查询确认模型是否支持相关能力。
