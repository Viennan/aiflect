...
文档类型：实现方案
日期：2026-05-05
引入需求：[REQ-2026-05-python-openai-adapter.CN.md](../../requirements/REQ-2026-05-python-openai-adapter.CN.md)

## 背景

Python 版本是 `aiflect` 的参考实现。第一阶段选择 OpenAI SDK 作为首个 provider adapter，用于验证 `aiflect` 的核心抽象是否能够落地到真实厂商 API。
...
- OpenAI API key 必须在初始化时显式传入，或通过 `ClientConfig.api_key` 提供；adapter 内部以 `SecretString` 保存。
...
工具调用输出和工具结果回填通过 `FunctionCallItem` 与 `FunctionResultItem` 表达。`aiflect` 不执行工具。
...
若未显式传入 `api_key` / `ClientConfig.api_key`，且未注入已有 SDK client，OpenAI adapter 会在初始化阶段报错；不再读取环境变量作为隐式凭据来源。
...
9. 补充单元测试并更新 [impls/python/STATUS.md](STATUS.md)。
