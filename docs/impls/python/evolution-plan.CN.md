...
截至 2026-05-28，Python 参考实现已完成 v0.4：
...
当前主要缺口已经从 core 抽象建设与第二 provider MVP 转向 hosted tools/media generation 与稳定化：

- provider-hosted tools、Remote MCP、图片/视频生成尚未进入稳定通用抽象。
- Media generation 与 provider-hosted tools 留给后续阶段。
...
3. 设计 media generation provider surface，优先覆盖 Volcengine image/video generation 的 task 与 stream 形态。
4. 增加默认关闭的真实 API integration test 约定，验证 Ark SDK 与文档之间的运行时差异。
5. 梳理 v0.4 后的 migration notes 和跨 provider 编程示例。
