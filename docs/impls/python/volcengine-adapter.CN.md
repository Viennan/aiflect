...
- OpenAI adapter 已完成 Responses generation/streaming、function/custom tools、structured output、text embedding、provider-native replay 与 remote context refresh。
...
用户仍传入完整 `GenerationRequest.items`。当用户提供 `RemoteContextHint(enable_cache=True, new_items_start_index=...)`，且边界前一个 item 的 provider snapshot metadata 中存在 response id 时，Volcengine adapter 可以只把新增 suffix 传给 provider，并通过 Ark 原生 `previous_response_id` 引用远端上下文。

如果 previous response 失效，adapter 会移除失效 response id，并用完整 `items` 自动 refresh 一次。该行为只针对明确的 previous response/context invalid 或 expired 错误，不扩展为通用请求重试。
...
RemoteContextHint.enable_cache -> store
snapshot response_id + RemoteContextHint.new_items_start_index -> previous_response_id + suffix input
...
当 `remote_context.enable_cache=True` 且 `remote_context.new_items_start_index` 指向有效新增边界时：

- `new_items_start_index` 必须小于等于 `len(items)`。
- adapter 检查 `items[new_items_start_index - 1]` 的 Volcengine Responses snapshot metadata。
- 找到 response id 时，provider 请求的 `input` 使用 `items[new_items_start_index:]`，并发送 Ark 原生 `previous_response_id`。
- 找不到 response id 时，provider 请求使用完整 `items`。
- 若 suffix 为空，adapter 应抛 `InvalidItemError`，避免发出无新增 input 的 Responses 请求。

Refresh 构造：

- optimized attempt：`previous_response_id` + suffix input。
- refresh attempt：移除 `previous_response_id`，用完整 `items` 重新构造 input。
- refresh 只在本次请求确实携带 `previous_response_id` 且错误可判断为 remote context 失效时执行一次。
...
只有本次请求实际携带 `previous_response_id` 时，才会在明确 remote context invalid/expired 错误下自动 refresh；普通错误不重试。
...
- refresh 到完整 input 时，完整 `items` 都需要按 replay policy 构造。
...
- function tool 声明。
- function result 回填。
- `RemoteContextHint(enable_cache=True, new_items_start_index=...)` 差分传输。
- anchor response id 缺失、边界越界、suffix 为空的行为。
...
- remote context refresh 只在本次请求携带 `previous_response_id` 且错误可判断为失效时发生。
...
- 实现 automatic remote context refresh。
