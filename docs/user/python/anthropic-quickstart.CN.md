...
本文说明 Python 版 `aiflect` 的 Anthropic provider 用法。完整 API 字段见 [api-reference.CN.md](api-reference.CN.md)，实现边界见 [anthropic-adapter.CN.md](../../impls/python/anthropic-adapter.CN.md)。
...
Anthropic Messages API 要求 `max_tokens`。使用 `aiflect` 时应通过 `GenerationConfig.max_output_tokens` 提供：

```python
from whero.aiflect import GenerationConfig, ImagePart, MessageItem, TextPart
from whero.aiflect.providers.anthropic import AnthropicClient
...
```python
from whero.aiflect import GenerationConfig, MessageItem, RemoteContextHint
...
```python
from whero.aiflect import GenerationConfig, MessageItem, ResponseFormat
...
Anthropic adapter 支持 user-executed function tools。`aiflect` 不自动执行工具；模型返回 `FunctionCallItem` 后，由用户代码执行工具，并在下一轮完整上下文中加入 `FunctionResultItem`。
