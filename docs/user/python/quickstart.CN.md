...
OpenAI、Volcengine 与 Anthropic client 也提供薄封装：
...
Anthropic adapter 会把 `ResponseFormat` 映射为 Messages API `output_config.format`；structured output 与 assistant message prefill 不兼容。
...
print(model_capability.supports_structured_output.value)
```
