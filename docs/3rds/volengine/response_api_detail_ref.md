...
input.content string / array  必选
用于生成回复的文本、图片、视频或文件输入，也可以包含先前助手的回复内容。
消息类型

input.role string 必选
...
input.arguments string 必选
要传递给函数的参数的 JSON 字符串。

input.call_id string 必选
模型生成的函数工具调用的唯一ID。

input.name string 必选
要运行的函数的名称。
...
仅模型 doubao-seed-1.8、deepseek-v3.2、doubao-seed-2.0支持设置思维链信息。
说明
...
说明
在请求中传入 previous_response_id，会引入上一轮请求的输入和回答内容，本次请求的输入tokens 会相应增加。工作原理可参见多轮对话场景。
...
deepseek-v4-pro-260425（待支持）
deepseek-v4-flash-260425（待支持）
...
tools.strict boolean 必选默认值 true
...
支持单一条件过滤和多条件组合过滤（支持And和Or逻辑运算）。详细使用方式和支持字段参见 filter 表达式。
说明
若自定义字段的类型为Boolean，则它的默认取值范围是 True/False，但大小写规则有所不同：在 cURL 中必须写作 true/false，在 Python 中必须写作 True/False。
