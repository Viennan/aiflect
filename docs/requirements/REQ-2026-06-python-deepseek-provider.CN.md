...
DeepSeek 同时提供 Anthropic 兼容接口与 OpenAI completion 兼容接口。当前 `vatbrain` 已有 Anthropic Messages API adapter，实现 DeepSeek Anthropic 兼容接口可以用较低成本补充 DeepSeek provider，并保持与现有 generation、streaming、function tools、reasoning 和 capability 模型一致。
...
- 新增 `whero.vatbrain.providers.deepseek` provider package。
- 新增 optional dependency：`whero-vatbrain[deepseek]`，当前复用 `anthropic` SDK。
