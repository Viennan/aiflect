...
| --- | --- | --- | --- | --- | --- |
| OpenAI response delta adapter options | `Completed` | `ClientConfig.adapter_options` 已作为 wrapper 行为配置字典引入；OpenAI adapter 可通过 `remote_context.response_delta=False` 禁用 `previous_response_id + suffix` 差分传输，同时保留完整上下文、`store=True` 与 `prompt_cache_key`；Volcengine/Anthropic/DeepSeek 不读取该配置 | [REQ-2026-07-openai-response-delta-adapter-options.CN.md](REQ-2026-07-openai-response-delta-adapter-options.CN.md) | [remote-context-cache-strategy.CN.md](../impls/python/remote-context-cache-strategy.CN.md), [api-reference.CN.md](../user/python/api-reference.CN.md), [quickstart.CN.md](../user/python/quickstart.CN.md) | 2026-07-06 |
