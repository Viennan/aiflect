# REQ-2026-05-python-reference-implementation-roadmap

状态：Completed
创建日期：2026-05-06
最近更新：2026-06-04
原始文档：`docs/impls/python/evolution-plan.CN.md`

## 需求定位

本文件由原 `docs/impls/python/evolution-plan.CN.md` 迁移而来，在新知识库标准下作为 Python 参考实现路线图的大需求管理文档。它记录 v0.2 到 v0.5 的需求级目标、阶段边界和完成状态；具体实现设计仍以 `docs/impls/python` 下的实现文档为准。

## 需求级进度

### 已完成子目标

- v0.2 Responses contract hardening 已完成。
- v0.3 Core API Family Expansion 已完成。
- Python Pydantic structured output helper 已完成。
- v0.4 Volcengine adapter MVP 已完成。
- v0.5 Media Generation 已完成。
- 对应实现设计、用户文档和 Python 实现状态已沉淀到知识库。

### 后续关注

- Provider capability matrix。
- 可选真实 API integration tests。
- migration notes 与错误处理 cookbook。
- Provider-hosted tools 专项设计。
- 跨 provider replay compatibility report。

## 背景

Python 是 `aiflect` 的参考实现语言。第一阶段已经完成基础包结构、core 基础模型、OpenAI provider client、OpenAI Responses API generation/streaming、function tool 映射、text embedding、usage 和 capability 基础表达。
...
- v0.3 Core API Family Expansion 已完成，见 [v0.3-core-api-family-expansion.CN.md](../impls/python/v0.3-core-api-family-expansion.CN.md)。
...
- v0.4 Volcengine adapter MVP 已完成，见 [volcengine-adapter.CN.md](../impls/python/volcengine-adapter.CN.md)。
...
- v0.5 Media Generation 已完成，见 [v0.5-media-generation.CN.md](../impls/python/v0.5-media-generation.CN.md)。
...
如果未来某个 provider 不提供 Responses 风格 API，才允许将 Chat/Completions 风格 API 映射到 `aiflect` generation 模型。
...
当 `previous_response_id` 或 provider-side context 失效时，Python client 不应默认静默重试。v0.3 已提供显式 `ReplayPolicy`：用户可以选择仅抛错、移除失效 remote context 后用完整 `items` 重放，或要求强制 provider-native replay。完整设计见 [provider-native-replay.CN.md](../design/provider-native-replay.CN.md)。
...
```text
python/whero/aiflect/core/generation.py
python/whero/aiflect/core/errors.py
python/whero/aiflect/providers/openai/mapper.py
python/whero/aiflect/providers/openai/stream.py
...
`FilePurpose` 已从 core 中移除；火山方舟 `purpose` 现阶段主要是 Files API 查询/上传的 provider-native 选项，尚不足以成为 aiflect 通用文件资源语义。
...
```text
python/whero/aiflect/core/items.py
python/whero/aiflect/core/generation.py
python/whero/aiflect/core/embeddings.py
python/whero/aiflect/core/resources.py
python/whero/aiflect/core/media.py
python/whero/aiflect/core/tools.py
...
详细实现方案见 [volcengine-adapter.CN.md](../impls/python/volcengine-adapter.CN.md)。本节保留 v0.4 的阶段边界、关键设计决策和验收口径，具体 mapper、Ark SDK 调用面、模块职责与测试矩阵以独立方案为准。

#### 设计输入

v0.4 方案基于以下知识库资料：

- [v0.3-core-api-family-expansion.CN.md](../impls/python/v0.3-core-api-family-expansion.CN.md)：当前 Python core 与 OpenAI adapter 基线。
- [provider-capability-integration.CN.md](../design/provider-capability-integration.CN.md)：API family、provider-side state、资源、embedding 与 hosted tools 边界。
- [provider-native-replay.CN.md](../design/provider-native-replay.CN.md)：provider-native snapshot、`ReplayPolicy` 与 remote context fallback。
- [response_api_text_gen.md](../3rds/volengine/response_api_text_gen.md)：Responses API 文本生成、上下文、streaming、store/cache。
- [response_api_multimodal_understanding.md](../3rds/volengine/response_api_multimodal_understanding.md)：Responses API 图片、视频、文档理解与 Files API 引用。
- [response_api_reasoning.md](../3rds/volengine/response_api_reasoning.md)：`thinking.type`、reasoning summary 与 `reasoning.effort`。
- [response_api_tool_calling.md](../3rds/volengine/response_api_tool_calling.md)：Responses API function calling 与 provider-hosted tools 对照。
...
```text
python/whero/aiflect/providers/volcengine/__init__.py
python/whero/aiflect/providers/volcengine/capabilities.py
python/whero/aiflect/providers/volcengine/client.py
python/whero/aiflect/providers/volcengine/mapper.py
python/whero/aiflect/providers/volcengine/stream.py
python/whero/aiflect/providers/volcengine/files.py
python/whero/aiflect/providers/volcengine/embeddings.py
python/tests/unit/test_volcengine_client.py
python/tests/unit/test_volcengine_generation_mapper.py
python/tests/unit/test_volcengine_stream_mapper.py
python/tests/unit/test_volcengine_files.py
...
状态：已完成。
目标：覆盖 OpenAI Images API 图片生成、Volcengine Ark 图片生成和 Volcengine 视频生成，并用两个 provider 的接口交集完善 media generation core model。OpenAI adapter 负责屏蔽 `images.generate` 与 `images.edit` 的 API 分裂，用户侧统一使用 `ImageGenerationRequest`。详细方案与实现记录见 [v0.5-media-generation.CN.md](../impls/python/v0.5-media-generation.CN.md)。
...
- `VideoGenerationRequest` 已进入 core media model，并从 `whero.aiflect` 与 `whero.aiflect.core` 导出。
...
火山方舟 SDK 支持部分本地路径便捷上传，但 `aiflect` 不应默认隐式上传。若实现便捷 API，必须是显式方法或显式 `auto_upload=True`。
