...
def test_anthropic_text_delta_maps_to_normalized_event() -> None:
    ...
    mapped = from_anthropic_stream_event(event, sequence=3)
...
def test_anthropic_tool_use_stream_rebuilds_function_call() -> None:
    accumulator = GenerationStreamAccumulator(provider="anthropic")
    events = [
        SimpleNamespace(
    ...
    for sequence, event in enumerate(events):
        accumulator.add(from_anthropic_stream_event(event, sequence=sequence))
...
def test_anthropic_stream_accumulator_rebuilds_text_response() -> None:
    accumulator = GenerationStreamAccumulator(provider="anthropic")
    events = [
        SimpleNamespace(
            type="content_block_delta",
...
def test_anthropic_usage_and_terminal_events_map() -> None:
    ...
    usage = from_anthropic_stream_event(usage_event, sequence=0)
    stopped = from_anthropic_stream_event(stop_event, sequence=1)
...
def test_anthropic_reasoning_error_and_unknown_events_map() -> None:
    reasoning = from_anthropic_stream_event(
        SimpleNamespace(
            type="content_block_delta",
            index=0,
    ...
    errored = from_anthropic_stream_event(
        {"type": "error", "error": {"message": "bad"}},
        sequence=1,
    )
    unknown = from_anthropic_stream_event({"type": "ping"}, sequence=2)
