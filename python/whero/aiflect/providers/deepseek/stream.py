"""DeepSeek Anthropic-compatible streaming event mapping."""
...
def from_deepseek_stream_event(event: Any, *, sequence: int) -> GenerationStreamEvent:
    """Convert one DeepSeek Anthropic-compatible stream event to an aiflect stream event."""
    ...
    if event_type == "message_delta":
        delta = _get_attr(event, "delta", None)
        usage = usage_from_deepseek(_get_attr(event, "usage", None))
