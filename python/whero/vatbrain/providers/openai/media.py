...
from whero.vatbrain.core.items import FilePart, ImagePart, MessageItem
...
def to_openai_image_params(
    ...
    *,
    ...
    """Convert a vatbrain image request into OpenAI Images API parameters."""
...
def from_openai_image_response(response: Any) -> ImageGenerationResponse:
    """Convert an OpenAI Images API response into a vatbrain response."""
...
def _image_part_to_openai_file(part: ImagePart, *, index: int) -> tuple[str, bytes, str]:
    if part.url is not None:
        raise UnsupportedCapabilityError(
            "OpenAI image edit requires file content; vatbrain does not download image URLs implicitly."
        )
