...
from whero.vatbrain.providers.deepseek.capabilities import (
...
from whero.vatbrain.providers.deepseek.stream import from_deepseek_stream_event


DEEPSEEK_API_FORMAT_ANTHROPIC = "anthropic"
DEEPSEEK_API_FORMAT_OPENAI_COMPLETION = "openai_completion"
DEFAULT_ANTHROPIC_BASE_URL = "https://api.deepseek.com/anthropic"


class DeepSeekClient:
    """Provider-level DeepSeek adapter client."""
    ...
    def __init__(
        ...
        *,
        ...
        api_format: str = DEEPSEEK_API_FORMAT_ANTHROPIC,
        model_capability_overrides: Mapping[str, Mapping[str, Any]] | None = None,
        **deepseek_client_options: Any,
    ) -> None:
        self.api_format = _normalize_api_format(api_format)
        if self.api_format == DEEPSEEK_API_FORMAT_OPENAI_COMPLETION:
            raise ValueError(
                "DeepSeek api_format='openai_completion' is reserved but not implemented yet."
            )
        self._client = client
        ...
        self._client_options = _merge_client_options(
            ...
            provider_options=deepseek_client_options,
    ...
    def generate(
        ...
        *,
        ...
        return from_deepseek_generation_response(response)
    ...
    async def agenerate(
        ...
        *,
        ...
        response = await self._acreate_generation_response(
            request,
            message="DeepSeek async generation request failed.",
        )
        return from_deepseek_generation_response(response)
    ...
    def stream_generate(
        ...
        *,
        ...
        stream = self._create_generation_stream(
            request,
            message="DeepSeek stream generation request failed.",
        )
        for sequence, event in enumerate(stream):
            yield from_deepseek_stream_event(event, sequence=sequence)
    ...
    async def astream_generate(
        ...
        *,
        ...
        stream = await self._acreate_generation_stream(
            request,
            message="DeepSeek async stream generation request failed.",
        )
        sequence = 0
    ...
    async def _acreate_generation_response(self, request: GenerationRequest, *, message: str) -> Any:
        params = to_deepseek_generation_params(request)
        try:
            return await self._async_client_instance.messages.create(**params)
        except Exception as exc:
            raise _provider_request_error(message, "messages.create", exc) from exc

    def _create_generation_stream(self, request: GenerationRequest, *, message: str) -> Any:
        params = to_deepseek_generation_params(request, stream=True)
        try:
            return self._sync_client.messages.create(**params)
        except Exception as exc:
            raise _provider_request_error(message, "messages.create", exc) from exc
...
def _normalize_api_format(api_format: str) -> str:
    normalized = str(api_format).strip().lower()
    supported = {
        DEEPSEEK_API_FORMAT_ANTHROPIC,
        DEEPSEEK_API_FORMAT_OPENAI_COMPLETION,
    }
    if normalized not in supported:
        choices = ", ".join(sorted(supported))
        raise ValueError(f"Unsupported DeepSeek api_format: {api_format!r}. Expected one of: {choices}.")
    return normalized
...
def _validate_client_credentials(
    *,
    ...
    if options.get("api_key") is not None:
        return
    raise ValueError(
        "DeepSeekClient requires api_key or ClientConfig.api_key at initialization "
